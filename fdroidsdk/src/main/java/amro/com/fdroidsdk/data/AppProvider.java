...
import amro.com.fdroidsdk.Preferences;
import amro.com.fdroidsdk.Utils;
import amro.com.fdroidsdk.data.Schema.ApkAntiFeatureJoinTable;
import amro.com.fdroidsdk.data.Schema.ApkTable;
import amro.com.fdroidsdk.data.Schema.AppMetadataTable;
import amro.com.fdroidsdk.data.Schema.AppMetadataTable.Cols;
import amro.com.fdroidsdk.data.Schema.AppPrefsTable;
import amro.com.fdroidsdk.data.Schema.CatJoinTable;
import amro.com.fdroidsdk.data.Schema.CategoryTable;
import amro.com.fdroidsdk.data.Schema.InstalledAppTable;
import amro.com.fdroidsdk.data.Schema.PackageTable;
import amro.com.fdroidsdk.data.Schema.RepoTable;
 ...
 *
 * As such, it is usually the case that you are interested in an {  App} which has its metadata
 ...
 *
 *  * {  AppProvider.Helper#findHighestPriorityMetadata(ContentResolver, String)}
 *
 * rather than:
 *
 *  * {  AppProvider.Helper#findSpecificApp(ContentResolver, String, long, String[])}
 *
 * The same can be said of retrieving a list of {  App} objects, where the metadata for each app
...
public class AppProvider extends FDroidProvider {
    ...
    public static final class Helper {
        ...
        /**
         * Returns an {  App} with metadata provided by a specific {@code repoId}. Keep in mind
     ...
     * {  org.fdroid.fdroid.data.QuerySelection#add(QuerySelection)} and
     * {  org.fdroid.fdroid.data.QuerySelection#add(String, String[])} methods
     * will only return the base class {  org.fdroid.fdroid.data.QuerySelection}
     * which is not aware of the installed app table.
     * However, the
     * {  org.fdroid.fdroid.data.AppProvider.AppQuerySelection#add(org.fdroid.fdroid.data.AppProvider.AppQuerySelection)}
     ...
     * of why you should usually prefer {  AppProvider#getHighestPriorityMetadataUri(String)} to
     * this method.
     */
    ...
    /**
     * Same as {  AppProvider#querySingle(String, long)} except it is used for the purpose
     * of an UPDATE query rather than a SELECT query. This means that it must use a subquery to get
     * the {  Cols.Package#PACKAGE_ID} rather than the join which is already in place for that
    ...
    /**
     * Helper method used by both the genuine {  AppProvider} and the temporary version used
     * by the repo updater ({  TempAppProvider}).
     */
     ...
     * (compared to {  AppProvider#updateAllAppDetails()}:
     *
     ...
     * {  android.app.IntentService} as described in https://gitlab.com/fdroid/fdroidclient/issues/520.
     */
