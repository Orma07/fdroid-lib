...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private void addIsAppToApp(SQLiteDatabase db, int oldVersion) {
        ...
        if (!columnExists(db, AppMetadataTable.NAME, AppMetadataTable.Cols.IS_APK)) {
            ...
            String query = "UPDATE " + AppMetadataTable.NAME + " SET " + AppMetadataTable.Cols.IS_APK + " = (" +
                    ...
                    "  WHERE " +
                    "    " + ApkTable.Cols.APP_ID + " = " + AppMetadataTable.NAME + "." + AppMetadataTable.Cols.ROW_ID + "    AND SUBSTR(" + apkName + ", LENGTH(" + apkName + ") - 3) != '.apk'" + ") = 0;";
            Log.i(TAG, query);
            db.execSQL(query);
        }
    }
     ...
     * and let the cache be filled from scratch by {InstalledAppProviderService}
     * For DB versions older than 43, this will create the {InstalledAppProvider}
